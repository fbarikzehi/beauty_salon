...
(function ($) {
    ...
    var jalaliDateTimePickerFlag = 'data-JalaliDateTimePicker',
        jalaliDateTimePickerFlagSelector = '[' + jalaliDateTimePickerFlag + ']',
        jalaliDateTimePickerGroupIdAttribute = 'data-JalaliDateTimePicker-group',
        jalaliDateTimePickerPopoverFlag = 'data-JalaliDateTimePicker-popover',
        jalaliDateTimePickerPopoverSelector = '[' + jalaliDateTimePickerPopoverFlag + ']',
        jalaliDateTimePickerContainerFlag = 'data-JalaliDateTimePicker-container',
        jalaliDateTimePickerContainerSelector = '[' + jalaliDateTimePickerContainerFlag + ']',
        mdPluginName = 'JalaliDateTimePicker',
        triggerStart = false;

    var popverHtmlTemplate = `
<div class="popover jalali-persian-datetime-picker-popover" role="tooltip" ${jalaliDateTimePickerPopoverFlag}>    
    <div class="arrow"></div>    
...
<div class="jalali-persian-datetime-picker-container {{rtlCssClass}}" ${jalaliDateTimePickerContainerFlag}>
...
<td class="border-0" style="{{monthTdStyle}}" {{monthTdAttribute}} data-td-month>
			...
			<tr {{theadSelectDateButtonTrAttribute}}>
                <td colspan="100" class="border-0">
                    <table class="table table-sm table-borderless">
                        <tr>
                            <th>
                                <button type="button" class="btn btn-light btn-sm" title="{{previousYearText}}" data-changedatebutton data-number="{{previousYearButtonDateNumber}}" {{previousYearButtonDisabledAttribute}}> {{previousYearIcon}} </button>
                            </th>
                            <th>
                                <button type="button" class="btn btn-light btn-sm" title="{{previousMonthText}}" data-changedatebutton data-number="{{previousMonthButtonDateNumber}}" {{previousMonthButtonDisabledAttribute}}> {{previousMonthIcon}} </button>
                            </th>
                            ...
                            <th>
                                <button type="button" class="btn btn-light btn-sm" title="{{nextMonthText}}" data-changedatebutton data-number="{{nextMonthButtonDateNumber}}" {{nextMonthButtonDisabledAttribute}}> {{nextMonthIcon}} </button>
                            </th>
                            <th>
                                <button type="button" class="btn btn-light btn-sm" title="{{nextYearText}}" data-changedatebutton data-number="{{nextYearButtonDateNumber}}" {{nextYearButtonDisabledAttribute}}> {{nextYearIcon}} </button>
                            </th>
                        </tr>
                    </table>
                </td>
    ...
    var previousYearTextPersian = 'سال قبل',
        ...
        minuteText = 'Minute',
        secondText = 'Second',
        previousYearIcon = '<i class="feather icon-chevrons-right"></i>',
        previousMonthIcon = '<i class="feather icon-chevron-right"></i>',
        nextMonthIcon = '<i class="feather icon-chevron-left"></i>',
        nextYearIcon = '<i class="feather icon-chevrons-left"></i>',
        amPm = {
            am: 0,
            pm: 1,
            none: 2
    ...
    function getPopoverDescriber($element) {
        //زمانی که روی داکیومنت اچ ام ال کلیک میشود تقویم بسته میشود
        // المانی را بر میگرداند که کاربر پلاگین را روی آن فعال کرده است
        var $popoverDescriber = $element.parents(jalaliDateTimePickerFlagSelector + ':first'); // inline
        // not inline
        if ($popoverDescriber.length <= 0) {
            $popoverDescriber = $element.parents(jalaliDateTimePickerPopoverSelector + ':first');
    ...
    function setPopoverHeaderHtml($element, isInLine, htmlString) {
        ...
        if (!isInLine) {
            if (isPopoverDescriber($element)) {
                ...
                $element.parents(jalaliDateTimePickerPopoverSelector + ':first').find('[data-name="mds-datetimepicker-title"]').html(htmlString);
            }
        } else {
            var $inlineTitleBox = $element.parents(jalaliDateTimePickerFlagSelector + ':first').find('[data-name="dateTimePickerYearsButtonsContainer"]');
    ...
    function updateCalendarHtml1($element, setting) {
        var calendarHtml = getDateTimePickerHtml(setting),
            $container = setting.inLine ?
                $element.parents(jalaliDateTimePickerFlagSelector + ':first') :
    ...
    function hideOthers($exceptThis) {
        $(jalaliDateTimePickerPopoverSelector).each(function () {
    ...
    function getYearsToSelectHtml(setting, yearToStart) {
        ...
        if ((setting.fromDate || setting.toDate) && setting.groupId) {
            var $toDateElement = $('[' + jalaliDateTimePickerGroupIdAttribute + '="' + setting.groupId + '"][data-toDate]'),
                $fromDateElement = $('[' + jalaliDateTimePickerGroupIdAttribute + '="' + setting.groupId + '"][data-fromDate]');
            if (setting.fromDate) {
        ...
        for (var i = yearStart; i < yearEnd; i++) {
            ...
            yearsToSelectHtml += `<td class="text-center" ${selectedDateTimeToShowJson.year == i ? 'selected-year' : ''}>
    ...
    function unbindEvents() {
        $(document).off('click', jalaliDateTimePickerContainerSelector + ' [data-day]');
        $(document).off('mouseenter', jalaliDateTimePickerContainerSelector + ' [data-day]');
        $(document).off('click', jalaliDateTimePickerContainerSelector + ' [data-changedatebutton]');
        $(document).off('blur', jalaliDateTimePickerContainerSelector + ' input[data-clock]');
        $(document).off('blur', jalaliDateTimePickerContainerSelector + ' input[data-clock]');
        $(document).off('click', jalaliDateTimePickerContainerSelector + ' [select-year-button]');
        $(document).off('click', '[data-yearrangebuttonchange]');
        $(document).off('click', jalaliDateTimePickerContainerSelector + ' [data-go-today]');
    ...
    // کلیک روی روزها
    $(document).on('click', jalaliDateTimePickerContainerSelector + ' [data-day]', function () {
        var $this = $(this),
        ...
        if (setting.rangeSelector) { // اگر رنج سلکتور فعال بود
            ...
            if (setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) {
                if (!setting.inLine) hidePopover($(jalaliDateTimePickerPopoverSelector));
        ...
        if (!setting.inLine) hidePopover($(jalaliDateTimePickerPopoverSelector));
            ...
            var $toDateDayElement = $('[' + jalaliDateTimePickerGroupIdAttribute + '="' + setting.groupId + '"][data-toDate]').find('[data-day]:first'),
                $fromDateDayElement = $('[' + jalaliDateTimePickerGroupIdAttribute + '="' + setting.groupId + '"][data-fromDate]').find('[data-day]:first');
    ...
    // هاور روی روزها
    $(document).on('mouseenter', jalaliDateTimePickerContainerSelector + ' [data-day],' + jalaliDateTimePickerContainerSelector + ' [data-nm],' + jalaliDateTimePickerContainerSelector + ' [data-pm]', function () {
        var $this = $(this),
    ...
    $(document).on('click', jalaliDateTimePickerContainerSelector + ' [data-changedatebutton]', function () {
        var $this = $(this),
            disabled = $this.attr('disabled'),
            dateNumber = Number($this.attr('data-number')),
            setting = getSetting1($this),
    ...
    // عوض کردن ساعت
    $(document).on('blur', jalaliDateTimePickerContainerSelector + ' input[data-clock]', function () {
        var $this = $(this),
            $thisContainer = $this.parents(jalaliDateTimePickerContainerSelector + ':first'),
    ...
    $(document).on('click', jalaliDateTimePickerContainerSelector + ' [select-year-button]', function () {
        var $this = $(this),
            setting = getSetting1($this),
            ...
            $jalaliDateTimePickerContainerSelector = $this.parents(jalaliDateTimePickerContainerSelector + ':first'),
            $dateTimePickerYearsToSelectContainer = $jalaliDateTimePickerContainerSelector.find('[data-name="dateTimePickerYearsToSelectContainer"]');
    ...
    $(document).on('click', '[data-yearrangebuttonchange]', function () {
        ...
        popoverHeaderHtml = popoverHeaderHtml.replace(/{{latestNextYear}}/img, yearsToSelectObject.yearStart > yearsToSelectObject.yearEnd ? yearsToSelectObject.yearStart : yearsToSelectObject.yearEnd);
        setPopoverHeaderHtml($this, setting.inLine, popoverHeaderHtml);
        $(jalaliDateTimePickerContainerSelector).find('[data-name="dateTimePickerYearsToSelectContainer"]').html(dateTimePickerYearsToSelectHtml);
    });

    // برو به امروز
    $(document).on('click', jalaliDateTimePickerContainerSelector + ' [data-go-today]', function () {
        var $this = $(this),
            setting = getSetting1($this);
    ...
    $('html').on('click', function (e) {
        ...
        hidePopover($(jalaliDateTimePickerPopoverSelector));
    });
    ...
    var methods = {
        init: function (options) {
            return this.each(function () {
                ...
                $this.attr(jalaliDateTimePickerFlag, '');
                ...
                if ((setting.fromDate || setting.toDate) && setting.groupId) {
                    $this.attr(jalaliDateTimePickerGroupIdAttribute, setting.groupId);
                ...
                if (setting.toDate && setting.fromDate) throw new Error(`JalaliDateTimePicker => You can not set true 'toDate' and 'fromDate' together`);
                if (!setting.groupId && (setting.toDate || setting.fromDate)) throw new Error(`JalaliDateTimePicker => When you set 'toDate' or 'fromDate' true, you have to set 'groupId'`);
                ...
                $(document).on('change', setting.targetTextSelector, function () {
                    ...
                    var $this1 = $(this),
                        value1 = $this1.val();
                    if (!value1) {
                        $this.JalaliDateTimePicker('clearDate');
                        return;
                    }
                    try {
                        if (!setting.rangeSelector)
                            $this.JalaliDateTimePicker('setDate', parseDateTime(value1, setting));
                        else {
                            let dateValues = value1.split(' - ');
                            $this.JalaliDateTimePicker('setDateRange', parseDateTime(dateValues[0], setting), parseDateTime(dateValues[1], setting));
                        }
                    } catch (e) {
                        setSelectedData(setting);
                    }
        ...
        getDateRange: function () {
            ...
            this.each(function () {
                ...
                var fromDateSetting = getSetting2($('[' + jalaliDateTimePickerGroupIdAttribute + '="' + setting.groupId + '"][data-fromDate]')),
                    toDateSetting = getSetting2($('[' + jalaliDateTimePickerGroupIdAttribute + '="' + setting.groupId + '"][data-toDate]'));
        ...
        setDate: function (dateTimeObject) {
            if (dateTimeObject == undefined) throw new Error('JalaliDateTimePicker => setDate => مقدار ورودی نا معتبر است');
            return this.each(function () {
                var $this = $(this),
        ...
        setOption: function (name, value) {
            if (!name) throw new Error('JalaliDateTimePicker => setOption => name parameter مقدار ورودی نا معتبر است');
            return this.each(function () {
                var $this = $(this),
                    setting = getSetting2($this);
                setting[name] = value;
        ...
        setDateRange: function (startDateTimeObject, endDateTimeObject) {
            if (startDateTimeObject == undefined || endDateTimeObject == undefined) throw new Error('JalaliDateTimePicker => setDateRange => مقدار ورودی نا معتبر است');
            if (convertToNumber4(startDateTimeObject) > convertToNumber4(endDateTimeObject)) throw new Error('JalaliDateTimePicker => setDateRange => مقدار ورودی نا معتبر است, تاریخ شروع باید بزرگتر از تاریخ پایان باشد');
            return this.each(function () {
                var $this = $(this),
                    setting = getSetting2($this);
                if (setting.rangeSelector) {
        ...
        setDatePersian: function (dateTimeObjectJson) {
            if (dateTimeObjectJson == undefined) throw new Error('JalaliDateTimePicker => setDatePersian => ورودی باید از نوه جی سان با حداقل پراپرتی های year, month, day باشد');
        ...
        destroy: function () {
            return this.each(function () {
                ...
                if (setting.inLine) {
                    $this.find(jalaliDateTimePickerContainerSelector).remove();
                }
                $this.removeAttr(jalaliDateTimePickerFlag)
                    .removeAttr('data-toDate')
                    .removeAttr('data-fromDate');
                $this.off(setting.trigger).popover('dispose');
                //if ($(jalaliDateTimePickerFlagSelector).length <= 1) {
    ...
    $.fn.JalaliDateTimePicker = function (method) {
        if (method['targetTextSelector'] == undefined) {
            method['targetTextSelector'] = "#" + $(this).prop('id');
        }
